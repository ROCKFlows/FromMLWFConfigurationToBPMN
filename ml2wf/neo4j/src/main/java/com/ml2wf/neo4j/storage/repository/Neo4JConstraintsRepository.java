package com.ml2wf.neo4j.storage.repository;

import com.ml2wf.contract.storage.graph.dto.GraphConstraintOperand;
import com.ml2wf.contract.storage.graph.repository.ConstraintsRepository;
import com.ml2wf.neo4j.storage.dto.Neo4JTaskVersion;
import org.springframework.data.neo4j.repository.Neo4jRepository;
import org.springframework.stereotype.Repository;

@Repository
public interface Neo4JConstraintsRepository extends Neo4jRepository<GraphConstraintOperand<Neo4JTaskVersion>, Long>,
        ConstraintsRepository<Neo4JTaskVersion, Long> {

}
